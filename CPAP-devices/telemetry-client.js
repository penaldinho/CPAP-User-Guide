(function () {
  ...
  const updateChatDockForTaskCardOverlap = () => {
    ...
    if (!callout || !isElementVisibleForLayout(callout)) {
      return;
    }

    if (!callout.dataset.mtgBaseWidth) {
      const computedCallout = window.getComputedStyle(callout);
      const parsedWidth = Number.parseFloat(String(computedCallout.width || '280'));
      callout.dataset.mtgBaseWidth = String(Number.isFinite(parsedWidth) ? parsedWidth : 280);
    }
  ...
  const presetTaskDescriptions = {
    ...
    scenario_card_2: {
      title: 'Scenario Card 2 – Fit and Start Therapy (Routine Use)',
        collapsedDescription: 'Fit the mask to the mannequin as outlined in the user guide, check and ensure mask fit is good using the Check Mask Fit option on the CPAP device, and start therapy.',
      steps: [
          'Fit the mask to the mannequin as outlined in the user guide, check and ensure mask fit is good using the Check Mask Fit option on the CPAP device, and start therapy.',
        'You may use the instructions at any time.'
      ]
    },
    scenario_card_3: {
